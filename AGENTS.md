...
- 设计功能逻辑的改动，需要维护`docs\CHANGELOG.md`，并记录更新时间。
...
- 文件上传使用 multer。
- 使用 pino 日志，避免散落 `console.log`。
- 文件上传使用 multer。

### 注释规范

- 统一使用中文撰写注释、错误消息和日志输出。
- 每个代码文件必须在顶部包含文件头注释，说明该模块的核心职责。
- 所有 `export` 函数、类公共方法、接口/类型定义必须使用 JSDoc（`/** ... */`）注释说明功能，建议使用 `@param` 描述参数、`@returns` 描述返回值。
- `private` 方法至少使用单行 JSDoc（`/** 一句话描述 */`）注释，或在其上方用 `//` 注释说明意图。
- 复杂逻辑内部推荐使用 `// 1. 步骤一`、`// 2. 步骤二` 形式的序列注释拆分步骤。
- 大型文件内使用 `// --- 类型定义 ---`、`// --- 公共 API ---` 形式的分隔线对相关代码分组。
...
```ts
/**
 * 根据用户输入生成 A2UI 消息，遇到校验失败自动进入修复循环（最多 3 次）。
 *
 * @param input - 用户文本或上传文件内容
 * @param context - Agent 上下文（对话历史、文件、schema 等）
 * @returns 校验通过后的 A2UI 消息，或最后一次修复失败的错误信息
 */
export async function generateA2UI(input: string, context: AgentContext): Promise<AgentResult> {
  // ...
}
```
