...
注意：原 Matt skills 不会直接生成本目录的 `README.md` 和 `scratch/README.md`。这两个文件是本仓库的路径迁移说明；`agents/issue-tracker.md`、`agents/domain.md`、`agents/triage-labels.md` 则对应 Matt setup skill 的三份配置文档。
...
2. 规格由 `cyz-to-spec` 发布到 `scratch/<feature-slug>/spec.md`。
3. 可执行 tickets 由 `cyz-to-tickets` 发布到 `scratch/<feature-slug>/issues/`。
