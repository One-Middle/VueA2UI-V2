...
5. 若后端返回 `workflow` 摘要（包括 `failed_retryable` 或 `interrupted` workflow 被普通消息恢复为 `running`），`workspace` 会先 `upsertWorkflow()` 更新本地状态；若返回 `agentRun` 或 workflow 已 running，则设置 `isGenerating = true`。
