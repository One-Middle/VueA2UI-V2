...
MessageInput
  ...
  -> workspace store -> renderer store -> PreviewPanel MessageProcessor -> SurfaceRuntime -> Vue Adapter 渲染
```
...
- `PreviewPanel.vue` 为每个 surface 创建和释放 `SurfaceRuntime`，并使用 Vue Adapter 消费快照；它不重新解释 A2UI 协议。
