...
- [x] 将 Button action 代码实现从历史扁平 `{ name, context }` 迁移到契约目标 `action.event`，同步 shared 类型、Agent schema、validateA2UI、Renderer 和测试。
- [x] 明确 `action.functionCall` 执行边界：仅作为未来能力保留在文档和 shared 类型中，当前 Agent schema 不放行，Renderer 不执行。
- [x] 核实 Renderer action/error 是否已由 Frontend 完整转发到 Backend；已补齐 Preview 宿主监听、API 调用和 Renderer action 回传测试。
